...
public:
  ...
  [[nodiscard]] constexpr auto GetRotationDegrees() const noexcept -> DefaultFloatType { return ToDegrees(angle_radians_); }
  [[nodiscard]] constexpr auto GetRotationRadians() const noexcept -> DefaultFloatType { return angle_radians_; }
  void SetRotationDegrees(DefaultFloatType angle_degrees) noexcept { angle_radians_ = ToRadians(angle_degrees); sin_ = std::sin(angle_radians_); cos_ = std::cos(angle_radians_); linear() = Matrix2{ {cos_, -sin_}, {sin_, cos_} } * Eigen::Scaling(scale_); }
  ...
  [[nodiscard]] auto GetSin() const noexcept -> DefaultFloatType { return sin_; }
  [[nodiscard]] auto GetCos() const noexcept -> DefaultFloatType { return cos_; }
